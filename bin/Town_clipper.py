...
def bitsToClip(bits):
    ...
    if len(bits)%BITS_PER_CHAR != 0:
        root.debug(f"Bit string length {len(bits)} must be a multiple of {BITS_PER_CHAR}")
...
class BitReader:
    ...
    def read(self, n, lenient=False):
        ...
        if self.get_remaining() < n and lenient is False:
            root.debug("Could not read {} bits (only {} remaining) at position {}".format(n, self.get_remaining(), self.cursor))

        readBits = self.bits[-self.cursor -n: -self.cursor if self.cursor > 0 else None]
        self.cursor += n
        if len(readBits) == 0:
            return 0
        else:
...
def bitsToDense(bits):
    ...
    if len(bits)%BITS_PER_CHAR != 0:
        root.debug("Bit string length {} must be a multiple of {}".format(len(bits), BITS_PER_CHAR))
        return
    ...
    for i in range(typeCount):
        ...
        if (typee > MAX_TYPE):
            root.debug("Invalid voxel type {}, max {} near bit {}".format(typee, MAX_TYPE, bitReader.cursor))
            return
    ...
    while bitReader.get_remaining() > 0:
        ...
        while (len(corner['voxels']) < MAX_HEIGHT):
            ...
            if (typeIndexZeroBased < EMPTY_TYPE or typeIndexZeroBased >= typeCount):
                ...
                root.debug("Invalid voxel type {}/{} at ({}, {}, {}) near bit {}".format(typeIndexZeroBased, typeCount, xPos, yPos, len(corner['voxels']) + 1,bitReader.cursor))
                return
...
def denseToBits(dense, pad = True):
    ...
    for corner in dense['corners']:
        maxXDelta = max(maxXDelta, corner['xDelta'] if corner['xDelta'] is not None else 0)
    ...
    outString = yOffsetBitLengthBits + xDeltaBitLengthBits + posBitLengthBits + outString
    ...
    # Corners
    cornersWithData = [corner for i, corner in enumerate(dense['corners']) if i==0 or corner['hasGround'] or len(corner['voxels']) > 0]

    # Ensure there's at least one (empty) corner
    # Townscaper encodes the empty map as AAAE, but without this we'd output AAAA
    corners = cornersWithData if len(cornersWithData) > 0 else [{"xDelta": None, "hasGround":False, "voxels": []}]
    isFirst = True
    ...
    for corner in corners:
        ...
        for typeIndexZeroBased in corner['voxels']:
            if (typeIndexZeroBased < EMPTY_TYPE or typeIndexZeroBased >= len(dense['types'])):
                root.debug(f"Invalid type {typeIndexZeroBased}, min 0, max {len(dense['types'])}")
                return
    ...
    # Padding
    if pad:
        paddingLength = (BITS_PER_CHAR - (len(outString) % BITS_PER_CHAR)) % BITS_PER_CHAR
...
def denseToSparse(dense):
    ...
    for denseCorner in dense['corners']:
        ...
        if (len(denseCorner['voxels']) > MAX_HEIGHT):
            root.debug(f"Too many voxels ({len(denseCorner['voxels'])}), max {MAX_HEIGHT}")
            return
        ...
        for h in range(len(denseCorner['voxels'])):
            ...
            if (typee < EMPTY_TYPE or typee >= len(dense['types'])):
                root.debug(f"Invalid voxel typee {typee} at ({x}, {y}, {h + 1}), min {EMPTY_TYPE}, max {len(dense['types']) - 1}")
                return
...
def sparseToDense(sparse):
    ...
    for sparseCorner in sparse:
        groundVoxel = sparseCorner['voxels'][GROUND_HEIGHT] if GROUND_HEIGHT in sparseCorner['voxels'] else None
    

        if (groundVoxel != None and groundVoxel != GROUND_TYPE):
            root.debug(f"Trying to set voxel typee {sparseCorner['voxels'][0]} on ground, only {GROUND_TYPE} allowed")
            return


        sparseCorner["hasGround"] = groundVoxel == GROUND_TYPE
        sparseCorner["voxels"] = {h:t for h,t in sparseCorner['voxels'].items() if t is not None}
    ...
    for sparseCorner in sortedSparseCorners:
        ...
        dense['corners'].append({'xDelta':xDelta,"yOffset":yOffset, "hasGround": sparseCorner["hasGround"], "voxels":voxels})
