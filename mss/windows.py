...
import sys
import threading
from ctypes import POINTER, WINFUNCTYPE, Structure, byref, c_void_p
from ctypes.wintypes import (ATOM, BOOL, DOUBLE, DWORD, HBITMAP, HDC, HGDIOBJ,
                             HWND, INT, LONG, LPARAM, LPCWSTR, LPRECT, RECT,
                             UINT, WORD)
...
class WINDOWINFO(Structure):
    ...
    """
    Structure that capture the info about a specified window
    """
...
CFUNCTIONS = {
    ...
    "FindWindowW": ("user32", [LPCWSTR, LPCWSTR], HWND),
    ...
    "GetWindowInfo": ("user32", [HWND, POINTER(WINDOWINFO)], BOOL),
    "GetWindowRect": ("user32", [HWND, LPRECT], BOOL),
    "PrintWindow": ("user32", [HWND, HDC, UINT], BOOL),
...
class MSS(MSSBase):
    ...
    __slots__ = {"_bbox", "_bmi", "_data", "_pwi", "fullscreen", "gdi32", "hwin", "window", "name", "user32"}
    ...
    def __init__(self, name, **_):
        ...
        #Name of the window to be fetched by mss
        self.name = name
    ...
    def _get_pid(self, name):
        """
        Retrieve the pid of the window name provided
        """
    ...
    def _get_window_dim(self):
        """
        Retrieve the dimension of the window to be captured
        v2 : the client area is returned instead of the fullwindow now. Fullscreen is initialized
        """
        # rect = RECT()
        # self.user32.GetWindowRect(self.hwin, ctypes.pointer(rect))
        pwi = WINDOWINFO()
        self._pwi = pwi
        self.user32.GetWindowInfo(self.hwin, byref(self._pwi))
        rect = self._pwi.rcClient   #Client area is used for more beautiful picture
        rect_all = self._pwi.rcWindow

        #Fullscreen
        if all([rect.left == rect_all.left, rect.top == rect_all.top, rect.right == rect_all.right, rect.bottom == rect_all.bottom]):
            self.fullscreen = True
        else:
    ...
    def _get_srcdc(self):
        ...
        if not srcdc and self.hwin:
            srcdc = MSS._srcdc_dict[cur_thread] = self.user32.GetDC(self.hwin)
    ...
    def update_window(self):
        """
        Update the value of window dimensions
        """
        self.hwin = self._get_pid(self.name)
        self.window = self._get_window_dim() if self.hwin else self.monitors[0]        
    ...
    def _grab_impl(self, monitor):
        ...
        if self.hwin:
            result = self.user32.PrintWindow(self.hwin, memdc, 3) #Might be necessary to set "1" or "0" instead
            if result != 1:
                raise ScreenShotError("user32.PrintWindow() failed.")
